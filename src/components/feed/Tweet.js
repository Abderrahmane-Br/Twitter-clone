...
import TweetHeading from './tweet/TweetHeading';
...
function Tweet({
    ...
    function linkify(text) {
        let descr = [];
        if (text) {
            let matches = Array.from(description.matchAll(/(http|https).*?(\s|$)/g));
            let start = 0;
            if (matches.length !== 0) {
                for (const match of matches) {
                    console.log(match);
                    descr.push(<>{text.slice(start, match.index)}</>)
                    descr.push(
                        <a
                            href={match[0]}
                            target="_blank"
                            className="tweet__description__link"
                        >
                            {match[0].replace(/(https|http):\/*/, "")}
                        </a>
                    )
